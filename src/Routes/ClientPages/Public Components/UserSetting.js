...
const UserSetting = () => {
  ...
  const handleChangePass = async () => {
    try {
      setIsLoad((prev) => ({ ...prev, changePassword: true }));
      ...
      if (
        UserData.newPass !== UserData.ReNewPass ||
        UserData.newPass.length !== UserData.ReNewPass.length
      ) {
        showToast(
          "error",
      ...
      const rs = await ResfulAPI.handleChangePass(
        UserData.oldPass,
        UserData.newPass,
        user.token
      );
      ...
      setIsLoad((prev) => ({ ...prev, changePassword: false }));
    }
  };
  ...
  const openModal = (code, data) => {
    if (code === "1") {
      setIsOpen((prev) => ({ ...prev, addAddress: true }));
    }
    if (code === "2") {
      setIsOpen((prev) => ({ ...prev, editAddress: true }));
  ...
  const closeModal = async (code, data) => {
    if (code === "1") {
      setIsOpen((prev) => ({ ...prev, addAddress: false }));
      await fetchAddress();
    }
    if (code === "2") {
      setIsOpen((prev) => ({ ...prev, editAddress: false }));
      await fetchAddress();
    }
  };
  ...
  const ProfileEdit = () => {
    return (
      <div className="bot">
        ...
        <AddnewAddress
          show={isOpen.addAddress}
          close={() => closeModal("1")}
          data={{ UserData: user.data.id, AddressData: userAdd }}
        />
        <AddressDetail
          show={isOpen.editAddress}
          close={() => closeModal("2")}
        ...
        <div className="d-flex">
          <div className="contentTopic">
            <div className="changePass">
              ...
              <p style={{ fontSize: "14px", fontWeight: "500", color: "gray" }}>
                Please enter your current password and a new password of your
                choice.
              </p>
              <input
                value={UserData.oldPass}
                onChange={(e) =>
                  setUserData((prev) => ({ ...prev, oldPass: e.target.value }))
                }
                type="password"
              ...
              <button
                disabled={isload.changePassword}
                onClick={handleChangePass}
                className={
                  isload.changePassword
                    ? "btn btn-secondary"
                    : "btn btn-success"
                }
              >
        ...
        <div className="banner02">
          <div className="Address">
            <div className="d-flex AddressHeader">
              ...
              {userAdd.length < 5 && (
                <button onClick={() => openModal("1")}>Add new address</button>
              )}
            </div>
            {userAdd.length !== 0 ? (
              <>
                        ...
                        >
                          <p style={{ fontWeight: "500" }}>
                            ...
                            {`${
                              ...
                              it.district ? it.district : "No district"
                            } - ${it.city ? it.city : "No City "}`}
                          </p>
                          <div>
                            <FontAwesomeIcon
