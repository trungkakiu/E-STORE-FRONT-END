    ...
    const fetchUserOrder = async () => {
        try {
            ...
            if (rs.status === 200) {
                ...
                showToast("Error", "Error while fetching user order history", "error", "CartHistories");
            }
        } catch (error) {
            showToast("Error", "Error while fetching user order history", "error", "CartHistories");
            console.error(error);
    ...
    const Cancel = async(orderID)  => {
        try {
            ...
            if (rs.status === 200) {
                ...
                showToast("Success", "Your order has been cancelled.", "success", "CartHistories");
                handleTabClick("Cancelled");
                return;
            } 
            if (rs.status === 400){
                showToast("Error", "Orders cannot be cancelled.", "error", "CartHistories");
                return;
            }else{
                showToast("Error", "Error while cancel user order", "error", "CartHistories");
                return;
            }
        } catch (error) {
            showToast("Error", "Error while cancel user order", "error", "CartHistories");
                                    ...
                                    <>
                                            ...
                                            <div key={index} className='Item'>
                                                <div className='d-flex top'>
                                                    ...
                                                    <p>Total: {order.total_price.toLocaleString("vi-VN")}đ</p>
                                                </div>
                                                <div className='bot d-flex flex-wrap'>
                                                    {
                                                        order.items?.map((it, id) =>(
                                                            <>
                                                            <div key={id} className='cardPr d-flex flex-wrap'>
                                                                <img style={{width: "4rem", height: "4rem"}} src={it.product_image ?
                                                                    `${IMGPATH}=${it.product_image}` 
                                                                    : "Product IMG"
                                                                } 
                                                                alt="Avatar"/>
                                                                <p style={{color: "gray"}}>{`x${it.quantity}`}</p>
                                                                <p>{it.product_name}</p>
                                                                <p style={{color: "orangered"}}>{`${(it.price.toLocaleString("vi-VN"))}đ`}</p>
                                                            </div>
                                                            </>
                                                        ))
                                                    }
                                                </div>
                                                {
                                                    order.status === "Pending" && (
                                                        <div className='optional'>
                                                            <button onClick={()=>Cancel(order.id)} className='btn btnCancer'>
                                                                Cancel order
                                                            </button>
                                                        </div>
                                                    )
                                                        ...
                                                        order.items?.map((it, id) =>(
                                                            ...
                                                            <div key={id} className='cardPr d-flex flex-wrap'>
                                                                <div className='d-flex flex-wrap'>
                                                                    <img style={{width: "5rem", height: "6rem"}} src={it.product_image ?
                                                                        `${IMGPATH}=${it.product_image}` 
                                                                        : "Product IMG"
                                                                    } 
                                                                    alt="Avatar"/>
                                                                    <p className='name' style={{marginTop: "30px"}}>{it.product_name}</p>
                                                                    <p style={{color: "orangered",marginTop: "30px"}}>{`${(it.price.toLocaleString("vi-VN"))}đ`}</p>
                                                                </div>
                                                                <RatingWithComment PrdID = {it.product_id} UserRatting={UserRatting}/>
                                                            </div>
                                                            </>
                                                        ))
                                                    }
