...
const SelectAddress = ({ show, close, data }) => {
    ...
    const openModal = (code, data ) =>{
        if(code === "1"){
            setModalState(
                (prev) => ({...prev, updateAdd: true})
            );
            setModalState(
                (prev) => ({...prev, updateAddData: data})
            )
            return;
        }
        if(code === "2"){
            setModalState(
                (prev) => ({...prev, addNewAdd: true})
            );
            return;
        }
    }
    ...
    const closeModal = async(code ) =>{
        if(code === "1"){
            setModalState(
                (prev) => ({...prev, updateAdd: false})
            );
            setModalState(
                (prev) => ({...prev, updateAddData: ""})
            )
        }
        if(code === "2"){
            setModalState(
                (prev) => ({...prev, addNewAdd: false})
            );
    ...
    const handleCloseModal = () =>{
        const inValid = modalData.filter(o => o.is_default === true)
        if(inValid){
            close();
        }else{
            showToast("Invalid-request", "Set any address as default before closing the interface.🐝", "error");
            return;
        }
    }
    return (
        <Modal show={show} onHide={handleCloseModal} animation={false}>
            <AddressDetail show={modalState.updateAdd} close={()=>closeModal("1")} data={{AddressData: modalState.updateAddData, onlyDefault: modalData.length < 2 ? true : false }}/>
            <AddnewAddress show={modalState.addNewAdd} close={()=>closeModal("2")} data={{AddressData: modalData}}/>
            <motion.div
                ...
                <Modal.Body>
                    ...
                    <div className="select-address">
                            ...
                            modalData?.length > 0  ? (
                                <>
                                    <div className="contents">
                                        {modalData?.sort((a, b) => (b.is_default ? 1 : 0) - (a.is_default ? 1 : 0))?.map((Item, id) => (
                                            <>
                                            <div key={id} className="d-flex cardIT">
                                                ...
                                                type="checkbox"/>
                                                <div style={{width: "100%"}}>
                                                    <div style={{justifyContent: "space-between"}} className="d-flex div_s87">
                                                        <div className="d-flex">
                                                            <p className="p_hs6">{Item.full_name}</p>
                                                            <p className="p_js6">{Item.phone_number}</p>
                                                        </div>
                                                        <p onClick={()=>openModal("1", Item)}
                                                        style={{paddingTop: "5px", color: "blue", marginRight: "20px", cursor: "pointer", userSelect: "none"}}>Update</p>
                                                    </div>
                                ...
                                <div className="contents-noaddress">
                                    <p>
                                        You currently do not have any shipping addresses, please add a new one now.
                                    </p>
                                    <button 
                                    onClick={()=>openModal("2")}>
                                        Add now
                        ...
                        <div className="footeModal d-flex">
                            <button onClick={handleCloseModal} className="btn btn-danger">Cancer</button>
                            <button onClick={handleCloseModal} className="btn btn-success">Confirm</button>
                        </div>
                    </div>
                </Modal.Body>
            </motion.div>
