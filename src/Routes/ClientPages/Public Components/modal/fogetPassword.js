...
const ForgotPassword = ({ show, close }) => {
    ...
    const SendEmail = async() =>{
        try {
            ...
            if(respone.status === 404){
                toast.warning("There is no account associated with your email." , {containerId: "top-right"})
                setIsLoad(false);
                return;
            }
        } catch (error) {
    ...
    const NextStep = ()=>{
        if(!UserInput.Email){
            toast.warning("Please enter Email that that received OTP before !" , {containerId: "top-right"})
            return;
        }
        setState(
            (prev)=>({...prev, state1: false, state2: true})
    ...
    const SendOTP = async() =>{
        try {
            setIsLoad(true);
            const respone = await ResfulAPI.SendOTP(UserInput.Email,UserInput.OTP, UserInput.NewPassword);
            if(respone.status === 200){
                toast.success("Password changed successfully !" , {containerId: "top-right"})
                setTimeout(()=>{
                    setIsLoad(false);
            ...
            if(respone.status === 422){
                setState(
                    (prev)=>({...prev, state2: false, state3: true})
                )
                setStateLine(
                    (prev) =>({...prev, state2: "LineOn", state3: "LineActive"})
                )
                setIsLoad(false);
                return;
            }
    ...
    const ChangePassWord = async() =>{
        try {
            if(UserInput.NewPassword.length < 6){
                toast.warning("Password must be at least 6 characters !" , {containerId: "top-right"})
                return;
            }
            setIsLoad(true);
            const respone = await ResfulAPI.SendOTP(UserInput.Email,UserInput.OTP, UserInput.NewPassword);
                            ...
                            <div className="d-flex stateLine">
                                ...
                                }
                                {
                                    stateLine.state2 !== "LineOn" ? (
                                        <>
                                        <div className={stateLine.state2 ? `${stateLine.state2}` : "LineOff"} style={{borderRadius: "0px"}}>
                                            <p>
                                                Code
                                            </p>
                                        </div>
                                ...
                                }
                                {
                                    stateLine.state3 !== true ? (
                                        <>
                                       <div className={stateLine.state3 ? `${stateLine.state3}` : "LineOff"} style={{borderTopLeftRadius: "0px", borderBottomLeftRadius: "0px"}}>
                                            <p>New</p>
                                        </div>
                            ...
                            state?.state1 === true && (
                                ...
                                <div className="State1">
                                    <div className="Header">
                                        <p>Get OTP if you don't have OTP, if you already have OTP enter the Email that received OTP</p>
                                    </div>
                        ...
                        }
                        {
                             state?.state2 === true && (
                                ...
                                <div className="State2">
                                    <div className="Header">
                                        <p>Please enter the OTP sent to Email: {UserInput.Email}</p>
                                    </div>
                                    ...
                                    <div className="d-flex">
                                    <p onClick={PrevStep} className="PrevStep"> {`< `}Send Mail</p>
                                    <button disabled={isLoad} onClick={()=>SendOTP()} className={isLoad? ("btn-load btn") : ("btn btn-on")}>
                            ...
                            state?.state3 === true && (
                                ...
                                >
                                    <div className="State3">
                                        <div className="Header">
                                            <p>Enter a new password for the account: {UserInput.Email}</p>
                                        </div>
                                        ...
                                        <button disabled={isLoad} onClick={()=>ChangePassWord()} className={isLoad? ("btn-load btn") : ("btn btn-on")}>
