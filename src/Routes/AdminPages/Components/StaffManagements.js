...
import { faSearch, faSpinner, faUserLargeSlash } from "@fortawesome/free-solid-svg-icons";
                ...
                {!StaffLoad ? (
                    <>
                        <div className="NoStaff">
                            <FontAwesomeIcon icon={faUserLargeSlash} className="NoStaff-icon"/>
                            <p className="P1Topic">Sorry, your system currently has no available staff.</p>
                            <p className="P2Detail">Please add staff if you want to use this function.</p>
                        </div>
                    </>
                ):(
                    <>
                        <div className="Staff row">
                            {
                                StaffLoad?.map((it, id)=>(
                                <>
                                    <div key={id} className="StaffCard">
                                        <div className="topcard">
                                            <img style={{width: "100%", height: "15.5rem"}} className="StaffIMG" src={it?.avatar_url ? (`${IMGPATH}=${it?.avatar_url  || "default.jpg"}`):(defautlIMG)} />
                                        </div>   
                                        <div className="bodycard">
                                            <p className="Fullname">{it.full_name? it.full_name : "No name"}</p>
