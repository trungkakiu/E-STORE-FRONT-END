...
const ProductImageUploader = ({ defaultImage, topicImg, otherImages, onUploadClick, IMGPATH }) => {
  return (
    <div className='uploadImg d-flex'>
      <div onClick={() => onUploadClick("topic")} style={{ cursor: "pointer" }} className='Upload9sa'>
        <img 
          src={topicImg ? URL.createObjectURL(topicImg) : (`${IMGPATH}=${defaultImage?.url || "default.jpg"}`)} 
          style={{ width: "100%", height: "100%", borderRadius: "10px", objectFit: "scale-down" }}
          alt="Product main"
        />
      </div>
      <div className='UploadO9ss'>
        <div onClick={() => onUploadClick("another")} className='addAnother' style={{ cursor: "pointer" }}>
          <FontAwesomeIcon icon={faPlus} className='ic9jjs' />
        </div>
        <div className='AnotherIMG'>
          {otherImages.defaultImages.map((img, id) => (
            <img key={`def-${id}`} src={`${IMGPATH}=${img?.url || "default.jpg"}`} className='card' alt={`Product ${id}`} />
          ))}
        </div>
      </div>
    </div>
...
const CategorySelector = ({ categories, selectedId, filter, onFilterChange, onSelect }) => {
  return (
...
const DetailProduct = () => {
  ...
  const { category, fetchProducts, fecthCategory } = useContext(ProductContext);
  ...
  const [modalState, setModalState] = useState({ editOtherIMG: false, deleteProduct: false });
  ...
  const defaultImage = useMemo(() => 
    ProductData?.images?.find(img => img.is_default) || ProductData?.images?.[0],
    [ProductData?.images]
  );
  ...
  const filterProduct = useMemo(() => 
    category.filter(item => item.name.toLowerCase().includes(CateFilter.toLowerCase())), 
    [category, CateFilter]
  );
  ...
  const handleFileChange = useCallback(async (code, event) => {
    try {
      ...
      const validFiles = files.filter(file => {
        ...
        if (!isValidType) {
          toast.error(`${file.name} không phải là file hình ảnh!`, { containerId: "edit" });
        }
        if (!isValidSize) {
          toast.error(`${file.name} quá lớn (tối đa 5MB)!`, { containerId: "edit" });
        }
      ...
      const toastId = toast.loading(`Đang tải: 0/${validFiles.length} ảnh`, { containerId: "edit" });
      ...
      const uploadPromises = validFiles.map(async (file) => {
        ...
        try {
          const rs = await ResfulAPI.UpLoadIMG(ProductData.id, formData, user.token, signal);
          completedUploads++;
          
          toast.update(toastId, { 
            render: `Đang tải: ${completedUploads + failedUploads}/${validFiles.length} ảnh (${failedUploads} lỗi)`, 
            containerId: "edit" 
          });
  ...
  const handleEditProduct = useCallback(async () => {
    ...
    try {
      const response = await ResfulAPI.handleEditProduct(ProductData, user.token);

      if (response.status === 200) {
        toast.success("Cập nhật thông tin sản phẩm thành công!", {containerId: ("edit")});
        return;
      } else if (response.status === 401) {
        toast.error("Bạn không có quyền thực hiện thao tác này! 🕵️", {containerId: ("edit")});
        return;
      } else if (response.status === 404) {
        toast.error("Sản phẩm không tồn tại! 🤓", {containerId: ("edit")});
        return;
      } else {
        toast.error("Có lỗi xảy ra khi cập nhật sản phẩm!", {containerId: ("edit")});
        return;
      }
    } catch (error) {
      console.error("Edit product error:", error);
      toast.error("Lỗi khi cập nhật thông tin sản phẩm!", {containerId: ("edit")} );
    } finally {
      setIsLoading(false);
    }
  ...
  const closeModal = useCallback(async(code) => {
    if (code === "1") {
        ...
        <div className="upleft"> 
          ...
          <p className='ptp0992'>General Information</p>
          {
            user.data.role === ("Admin") && (
              <FontAwesomeIcon onClick={()=>openModal("2")} className='icon' icon={faTrash}/>
            )
          }
          </div>
          
          <label className='lb776s' htmlFor="product-name">Name Product</label>
          <input 
            id="product-name"
            className='ipn77s'
            ...
            <p className='pK982s'>Product Image</p>
            {
                otherImages.length !== 0 ? (
                    ...
                    style={{
                      marginLeft: "53%", 
                      fontSize: "30px", 
                      color: ProductData.images?.some(img => !img.is_default) || otherImages.length > 0 ? "black" : "gray"
                    }} 
                    className='coas'
                    icon={faEdit} 
