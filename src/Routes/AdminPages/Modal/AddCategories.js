...
const AddCategory = ({ show, close, data }) => {
    ...
    const { category, products, addCategory, addProduct, removeProduct, updateProduct, fecthCategory } = useContext(ProductContext);
    ...
    const addNewCategory = async() =>{
        try {
            console.log("dt: ",data)
            const resopne = await ResfulAPI.AddCategory({cateName,parent_id } ,user.token);
                ...
                <Modal.Body>
                    ...
                    <div className="addCategoris">
                        ...
                        <div className="inputraw">
                            ...
                            <input value={cateName} onChange={(e)=>setCategoryName(e.target.value)}/>
                            <div className="category-parent">
                                ...
                                <div className="listcate">
                                    {data?.map((item, index) =>(
                                        <div 
                                            key={index} 
                                            className={`item ${selectedIndex === index ? "itemchoose" : ""}`} 
                                            onClick={() => {
                            ...
                            <div className="d-flex button">
                                <div onClick={()=>close()} className="btn cancel">Cancel</div>
                                <div onClick={addNewCategory} className="btn success">Summit</div>
                            </div>
                        </div>
